...
%         end
        ...
        for idxcoeff = 1:size(coeff_tests,2)
            size_rep = size(gtcc(svm_params.X_train(1,:)', fs, NumCoeffs=coeff_tests(1,idxcoeff)));
            ...
            parfor i = 1:size(X_train_representation,1)
                segment_rep = gtcc(svm_params.X_train(i,:)', fs, NumCoeffs=coeff_tests(1,idxcoeff));
            ...
            parfor i = 1:size(X_test_representation,1)
                segment_rep = gtcc(svm_params.X_test(i,:)', fs, NumCoeffs=coeff_tests(1,idxcoeff));
                segment_rep = reshape(segment_rep,[], size_rep);
            ...
            unique_classes = unique_classes(1:10);
            pos = ismember(y_train, unique_classes);
            svm_train.X_train = X_train(pos,:);
            svm_train.y_train = y_train(pos);

            pos = ismember(y_test, unique_classes);
            svm_train.X_test = X_test(pos,:);
            svm_train.y_test = y_test(pos);
