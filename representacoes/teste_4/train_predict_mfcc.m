...
%         end
        ...
        for idxcoeff = 1:size(coeff_tests,2)
            size_rep = size(mfcc(svm_params.X_train(1,:)', fs, NumCoeffs=coeff_tests(1,idxcoeff)));
            ...
            parfor i = 1:size(X_train_representation,1)
                segment_rep = mfcc(svm_params.X_train(i,:)', fs, NumCoeffs=coeff_tests(1,idxcoeff));
            ...
            parfor i = 1:size(X_test_representation,1)
                segment_rep = mfcc(svm_params.X_test(i,:)', fs, NumCoeffs=coeff_tests(1,idxcoeff));
                segment_rep = reshape(segment_rep,[], size_rep);
