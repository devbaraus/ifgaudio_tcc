...
for sec = [3]
    ...
    for intensidade_idx = 1:size(intensidade,2)
        ...
        for idxcoeff = 1:size(order_tests,2)
            size_rep = size(fft(svm_params.X_train(1,:)', order_tests(1,idxcoeff))');
            ...
            parfor i = 1:size(X_train_representation,1)
                segment_rep = abs(fft(svm_params.X_train(i,:)', order_tests(1,idxcoeff))');
            ...
            parfor i = 1:size(X_test_representation,1)
                segment_rep = abs(fft(svm_params.X_test(i,:)', order_tests(1,idxcoeff))');
                %                 segment_rep = reshape(segment_rep,[], size_rep);
