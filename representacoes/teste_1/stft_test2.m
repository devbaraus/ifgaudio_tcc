...
stfts = zeros(286720, numArquivos);
for aa = 1:numArquivos
    ...
    audioIn1 = audioIn1(1:fs1*3);                                   % pega os primeiros 3 segundos do arquivo de audio
    coeffs = abs(stft(audioIn1,"Window",win,"OverlapLength",512));  % aplica-se a representacao no arquivo
	stfts(1:286720, aa) = reshape(coeffs.', [], 1);					% aplica-se a representacao no arquivo e guarda o resultado num array
end
...
arquivoSelecionado = arquivoSelecionado(1:fs1*3);
...
for potRelativa = intensidadePotenciaRuido
	...
	for i = 1:numExperimentos
		...
		SNRMedia = SNRMedia + snr(arquivoSelecionado, ruidoAditivo);				% acumula-se o valor de SNR para depois calcular a SNR media ao final dos experimentos para um valor de SNR
		repArquivo = abs(stft(arquivoSelecionado,"Window",win,"OverlapLength",512));% aplica a representacao no arquivo de "inferencia" infectado por ruido branco
        repArquivo = reshape(repArquivo.', [], 1);
		erroREP = sum( abs((stfts - repmat(repArquivo, 1, numArquivos)).^2), 1);		% calcula a diferenca entre a fft do audio ruidoso e as ffts de cada audio da "base de dados de treinamento"
		[~, idx] = min(erroREP);													% faz um processo de maxima-verossimilhanca.
		...
		if idx == idxArquivoSelecionado
			acerto = acerto + 1;						% o arquivo selecionado na inferencia eh o arquivo esperado? anota o acerto.
		end
	end
...
title('Representação STFT');
ylim([0,100]);
grid on;
saveas(gcf,'sftf_graph.png');