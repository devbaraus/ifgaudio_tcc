...
%% Aplica uma representacao (neste caso, LPC) a todos os arquivos do "treinamento"
lpcs = zeros(1025, numArquivos);
parfor aa = 1:numArquivos
	...
	lpcs(:, aa) = reshape(lpc( audioIn1, 1024 ).', [],1);						% aplica-se a representacao no arquivo e guarda o resultado num array
end
...
for potRelativa = intensidadePotenciaRuido
	...
	for i = 1:numExperimentos
		...
		repArquivo(:, 1) = lpc(arquivoSelecionado + ruidoAditivo, 1024);			% aplica a representacao no arquivo de "inferencia" infectado por ruido branco
		erroREP = sum( abs((lpcs - repmat(repArquivo, 1, numArquivos)).^2), 1);		% calcula a diferenca entre a fft do audio ruidoso e as ffts de cada audio da "base de dados de treinamento"
...
title('Representação LPC');
ylim([0,100]);
grid on;
saveas(gcf,'lpc_graph.png');