...
parfor aa = 1:numArquivos
    ...
    coeffs = mfcc(audioIn1, fs1);                                   % aplica-se a representacao no arquivo
	mfccs(:, aa) = reshape(coeffs.', [], 1);                   % faz o reshape (1, 4172) e guarda o resultado num array
end
...
for potRelativa = intensidadePotenciaRuido
	...
	for i = 1:numExperimentos
		...
		repArquivo = mfcc(arquivoSelecionado + ruidoAditivo, fs2);			% aplica a representacao no arquivo de "inferencia" infectado por ruido branco
        repArquivo = reshape(repArquivo.', [], 1);
		erroRep = sum( abs((mfccs - repmat(repArquivo, 1, numArquivos)).^2), 1);		% calcula a diferenca entre a fft do audio ruidoso e as ffts de cada audio da "base de dados de treinamento"
...
title('Representação MFCC');
ylim([0,100]);
grid on;
saveas(gcf,'mfcc_graph.png');