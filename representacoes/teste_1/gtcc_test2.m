...
numArquivos = size(xx, 1);D

%% Aplica uma representacao (neste caso, GTCC) a todos os arquivos do "treinamento"
gtccs = zeros(4172, numArquivos);
parfor aa = 1:numArquivos
    ...
    coeffs =  gtcc(audioIn1, fs1);                                  % aplica-se a representacao no arquivo
	gtccs(:, aa) = reshape(coeffs.', [], 1);					% faz o reshape (1, 4172) e guarda o resultado num array
end
...
arquivoSelecionado = arquivoSelecionado(1:fs2*3);
...
for potRelativa = intensidadePotenciaRuido
	...
	parfor i = 1:numExperimentos
		...
		repArquivo = gtcc(arquivoSelecionado + ruidoAditivo, fs1);                  % aplica a representacao no arquivo de "inferencia" infectado por ruido branco
        repArquivo = reshape(repArquivo.', [], 1);
		erroREP = sum( abs((gtccs - repmat(repArquivo, 1, numArquivos)).^2), 1);	% calcula a diferenca entre a fft do audio ruidoso e as ffts de cada audio da "base de dados de treinamento"
...
title('Representação GTCC');
ylim([0,100]);
grid on;
saveas(gcf,'gtcc_graph.png');