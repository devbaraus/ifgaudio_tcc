...
tamREP = prod(size(lpc(zeros(tamanhoFsSegmento,1),1024)));
...
for aa = 1:numArquivos
    ...
    parfor i = 1:qtdSegmentosArr(aa,1)
        ...
        repAudiosMat(:,aa,i) = reshape(lpc(segmentoAudio, 1024).', [], 1); 
    end
end
...
for potRelativa = intensidadePotenciaRuido
    ...
    parfor i = 1:numExperimentos
        ...
        repArquivo = reshape(lpc(arquivoSelecionado + ruidoAditivo, 1024).', [], 1);
...
ylim([0 100]);
title(sprintf('Representação LPC (%i experimentos)', numExperimentos));
grid on;
saveas(gcf,sprintf('lpc_graph_%i.fig', numExperimentos));
close(f);