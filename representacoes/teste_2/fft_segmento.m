...
for aa = 1:numArquivos
    ...
    parfor i = 1:qtdSegmentosArr(aa,1)
        ...
        repAudiosMat(:,aa,i) = fft(segmentoAudio, tamFFT);
    end
end
...
for potRelativa = intensidadePotenciaRuido
    ...
    for i = 1:numExperimentos
        ...
        fftArquivo = fft(arquivoSelecionado + ruidoAditivo, tamFFT);
        SNRMedia(i) = snr(arquivoSelecionado, ruidoAditivo);
        
        for j = 1:maxQtdSegmentos
            erroFFT = sum( abs((repAudiosMat(:,:,j) - repmat(fftArquivo, 1, numArquivos)).^2), 1);
            [~, idx] = min(erroFFT);
