...
tamREP = prod(size(gtcc(zeros(tamanhoFsSegmento,1),fs)));
...
for aa = 1:numArquivos
    ...
    parfor i = 1:qtdSegmentosArr(aa,1)
        ...
        repAudiosMat(:,aa,i) = reshape(gtcc(segmentoAudio, fs).', [], 1); 
    end
end
...
for potRelativa = intensidadePotenciaRuido
    ...
    parfor i = 1:numExperimentos
        ...
        repArquivo = reshape(gtcc(arquivoSelecionado + ruidoAditivo, fs).', [], 1);
...
ylim([0 100]);
title(sprintf('Representação GTCC (%i experimentos)', numExperimentos));
grid on;
saveas(gcf,sprintf('GTCC_graph_%i.fig', numExperimentos));
close(f);